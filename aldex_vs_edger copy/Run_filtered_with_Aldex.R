...
# ALDEx3 still runs on all 1117 OTUs (needs full set for normalization),
# then we subset its results with the keep vector.
...
out_dir   <- "~/Desktop/Western_COOP/Results_COOP/aldex_vs_edger/filtered"
gamma_val <- 1e-05
nsample   <- 128
...
for (i in 1:ncol(pairs)) {
  ...
  # edgeR TMM with filterByExpr
  group <- factor(conds)
  dge   <- DGEList(counts = Y, group = group)
  keep  <- filterByExpr(dge, group = group)
  cat(sprintf("  kept %d / %d OTUs\n", sum(keep), nrow(Y)))
  ...
  # subset ALDEx3 to match (keep vector, same row order as Y)
