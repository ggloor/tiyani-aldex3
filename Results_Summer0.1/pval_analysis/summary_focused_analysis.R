...
# ---- Main output folder -----------------------------------------------------
maindir <- "/Users/pranavdivvela/Desktop/3383/0_git/tiyani-aldex3/Results_Summer0.1/pval_analysis0.2"
...
plot_effect_pdf <- function(pd_list, comp, label, plotdir, gamma_val) {
  ...
  for (item in pd_list) {
    ...
    n_nonsig_left  <- sum(pd$neither) + sum(pd$clr_only)
    n_nonsig_right <- sum(pd$neither) + sum(pd$tss_only)

    par(mfrow = c(1, 2))

    # ---- Left panel: CLR results, TSS sig overlay ----
    par(mar = c(5, 5, 4, 2))
    nonsig_left <- pd$neither | pd$clr_only
    plot(pd$se_clr[nonsig_left], pd$est_clr[nonsig_left],
         col = "grey70", pch = 1, cex = 0.5,
         xlim = range(pd$se_clr, na.rm = TRUE),
         ylim = range(pd$est_clr, na.rm = TRUE),
         xlab = "Std Error", ylab = "Estimate",
         main = paste0(comp, " - ", clr_lab, " (", tss_lab, " sig overlay)"),
    ...
    legend("topleft",
           legend = c(paste0("Both (", n_both, ")"),
                      paste0("TSS only (", n_tss, ")"),
                      paste0("Non-sig (", n_nonsig_left, ")")),
           col = c("red", "blue", "grey70"),
           pch = c(16, 16, 1), cex = 0.85, bg = "white")

    # ---- Right panel: TSS results, CLR sig overlay ----
    par(mar = c(5, 5, 4, 2))
    nonsig_right <- pd$neither | pd$tss_only
    plot(pd$se_tss[nonsig_right], pd$est_tss[nonsig_right],
         col = "grey70", pch = 1, cex = 0.5,
         xlim = range(pd$se_tss, na.rm = TRUE),
         ylim = range(pd$est_tss, na.rm = TRUE),
         xlab = "Std Error", ylab = "Estimate",
         main = paste0(comp, " - ", tss_lab, " (", clr_lab, " sig overlay)"),
         cex.lab = 1.2, cex.main = 0.9)
