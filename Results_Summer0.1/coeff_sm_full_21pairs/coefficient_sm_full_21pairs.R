...
#
# Uses coefficient.sm with c.sd (std dev per coefficient) per ALDEx3 v1.3.0:
#   c.mu = c(0, k)        - intercept=0, slope=k
#   c.sd = c(0, gamma)    - no intercept uncertainty, gamma on slope
#
# 5 unified categories: all3 (red), coeff only (orange), CLR only (dodgerblue),
...
#
# Outputs per gamma:
#   data/   - summary_stats, slope_table, red_dot_details CSVs
#   plots/  - regression, scatter, effect PDFs per comparison
# =============================================================================
...
# ---- Output -----------------------------------------------------------------
maindir <- "/Users/pranavdivvela/Desktop/3383/0_git/tiyani-aldex3/Results_Summer0.1/coeff_sm_full_21pairs/"
dir.create(maindir, showWarnings = FALSE, recursive = TRUE)

# ---- Load all 7 cohorts -----------------------------------------------------
cohort_names <- c("cent", "eld", "kin", "mage", "mid", "pup", "you")
datasets <- list()
for (name in cohort_names) {
  load(paste0(datadir, name, ".Rda"))
  datasets[[name]] <- get(name)
}

# ---- Build all 21 pairs -----------------------------------------------------
pairs_list <- list()
for (i in 1:(length(cohort_names) - 1)) {
  for (j in (i + 1):length(cohort_names)) {
    pairs_list[[length(pairs_list) + 1]] <- c(cohort_names[i], cohort_names[j])
  }
}
cat("Total pairs:", length(pairs_list), "\n")
...
run_analysis <- function(sum_coeff, sum_clr, sum_tss, k, comp) {
  ...
  stats_row <- data.frame(
    ...
    n_tss_only        = n_tss,
    n_none            = n_none,
    n_sig_coeff       = sum(sig_coeff),
    n_sig_clr         = sum(sig_clr),
    n_sig_tss         = sum(sig_tss),
  ...
  # Slope/intercept on red dots (all3)
  all3_idx       <- cats == "all3"
...
# ---- Helper: regression plots per comparison --------------------------------
...
# #############################################################################
# MAIN LOOP: gamma x constant x pair
# #############################################################################
...
for (gamma_val in gammas) {
  ...
  for (k in constants) {
    ...
    for (pr in pairs_list) {
      ...
      # Coefficient model (ALDEx3 v1.3.0 c.sd API)
...
# ---- Copy this script into the results folder for reproducibility -----------
script_src <- "/Users/pranavdivvela/Desktop/3383/0_git/tiyani-aldex3/code/coefficient_sm_full_21pairs.R"
if (file.exists(script_src)) {
  file.copy(script_src, paste0(maindir, "coefficient_sm_full_21pairs.R"),
            overwrite = TRUE)
  cat("Script copied to results folder\n")
}

cat("\n========== All done! ==========\n")
cat("Main folder:", maindir, "\n")
cat("Structure: gamma_{0.0001,0.3,0.5}/{data,plots}/\n")
cat("Total comparisons: 21 pairs x 21 constants x 3 gammas\n")
