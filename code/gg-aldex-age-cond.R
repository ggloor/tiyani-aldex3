...
age <- c(as.numeric(kin.age[1,]), as.numeric(cent.age[1,]))
conds <- c(rep("k", ncol(kin.age)), rep("C", ncol(cent.age)))
...
res.age <- ALDEx3::aldex(Y, ~age, data, nsample=128, scale=clr.sm, gamma=0.5)
...
points(sum.0a$std.error[siga], sum.0a$estimate[siga], col='darkgreen', pch=19, cex=0.5)
abline(h=0)