# 01_run_unfiltered.R
# Run ALDEx3 (CLR + TSS) and edgeR TMM (QL pipeline) on all 1117 OTUs.
# Inf rows from ALDEx3 MC sampling are removed before saving.
# Saves one .Rda per comparison with matched OTU sets.
...
out_dir   <- "~/Desktop/Western_COOP/Results_COOP/aldex_vs_edger/unfiltered_QuickStart"
gamma_val <- 1e-05
nsample   <- 128
...
for (i in 1:ncol(pairs)) {
  ...
  # --- edgeR QL pipeline (quick start, no filtering) ---
  group  <- factor(conds)
