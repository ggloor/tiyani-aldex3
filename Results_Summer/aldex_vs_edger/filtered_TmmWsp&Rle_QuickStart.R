# 02b_run_filtered_other_norms.R
# Same as 01b but edgeR uses filterByExpr first.
# ALDEx3 runs on the full OTU set (needs complete composition),
# then results are subset to match edgeR-kept OTUs.
# Filtering is done once; only the normalization varies per loop.
...
for (norm in edger_norms) {
  dir.create(file.path(base_out, paste0("filtered_", norm)),
...
for (i in 1:ncol(pairs)) {
  ...
  # --- filterByExpr (done once, shared across norms) ---
  group  <- factor(conds)
  ...
  # --- Subset ALDEx3 to match kept OTUs ---
  ...
  for (norm in edger_norms) {
    ...
    dge_n  <- normLibSizes(dge, method = norm)
    design <- model.matrix(~group)
    fit    <- glmQLFit(dge_n, design)
    ...
    out_dir <- file.path(base_out, paste0("filtered_", norm))
